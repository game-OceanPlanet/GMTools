<template>
  <div>
    ...
    <Form :model="formModel" :label-width="80">
      ...
      <FormItem label="增减数量">
        <Input v-model="formModel.changeCount" placeholder="输入需要增加或减少的货币数量"></Input>
      </FormItem>

       <Button type="primary" @click="handleSubmit">充值</Button>
       <br>
       <br>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchOperateList() {
        this.operateList = [];
        let moneyNames = ["金币","USDT","KAD","KEY","购买名额","加速积分","海豚金币","未开发","未开发","未开发"];
      ...
      handleSubmit() {
        ...
        let changeCount = parseInt(this.formModel.changeCount);

        services.getHttpClient().post({
            url: "/dragon/currencyEdit",
            body: {
